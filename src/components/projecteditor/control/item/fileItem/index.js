...
export default class FileItem extends Item {
    ...
    _clickRenameFile = e => {
        ...
        if (newFile) {
            ...
            // Check if user is trying to move directory into a sub directy, an action we can't support.
            const forbiddenPrefix = this.getFullPath() + '/';
            if (newFile.indexOf(forbiddenPrefix) === 0) {
                alert("Error: Could not move directory into it's own sub directory");
                return
            }
