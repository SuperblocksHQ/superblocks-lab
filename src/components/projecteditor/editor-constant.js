...
export default class AccountConstant extends Component {
    constructor(props) {
        ...
        this.constant = this.dappfile.getItem('constants', [
            { name: props.constant },
        ]);
        this.form = { env: '' };
    }
    ...
    save = e => {
        ...
        if (!this.constant.obj.name.match(/^([a-zA-Z0-9-_]+)$/)) {
            alert(
                'Illegal constant name. Only A-Za-z0-9, dash (-) and underscore (_) allowed.'
            );
            return;
        }
        if (
            !this.dappfile.setItem(
                'constants',
                [{ name: this.props.constant }],
                this.constant
            )
        ) {
            alert(
                'Dappfile.yaml updated. You need to reload projects before saving.'
            );
            return;
        }
    ...
    render() {
        ...
        if (!this.constant) {
            return (
                <div>
                    Could not find constant {this.props.constant} in
                    Dappfile.yaml
                </div>
            );
        }
